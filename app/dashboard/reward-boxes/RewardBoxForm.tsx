...
import { rewardBoxSchema, RewardBoxFormValues } from '@/schemas/rewardBox.schema';
...
export function RewardBoxForm({ rewardBox, onFinished }: RewardBoxFormProps) {
    ...
    const form = useForm<RewardBoxFormValues>({
        ...
        defaultValues: {
            ...
            type: rewardBox?.type || 'common',
        },
    });

    const onSubmit = (values: RewardBoxFormValues) => {
        if (isEditMode) {
            updateMutation.mutate({ id: rewardBox.id, ...values }, {
    ...
    return (
        <div>
            <h2 className="text-2xl font-semibold mb-4">{isEditMode ? 'Edit Reward Box' : 'Create New Reward Box'}</h2>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Textarea placeholder="A common box with a chance for basic rewards." {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                    ...
                    <div className="flex space-x-4">
                        <Button type="submit" disabled={isPending}>{isPending ? 'Saving...' : 'Save Box'}</Button>
