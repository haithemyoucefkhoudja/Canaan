...
export function RewardForm({ reward, onFinished }: RewardFormProps) {
    ...
    const form = useForm<RewardFormValues>({
        ...
        defaultValues: {
            ...
            icon: reward?.icon || 'https://picsum.photos/50',
    ...
    const onSubmit = (values: RewardFormValues) => {
        if (isEditMode) {
            updateMutation.mutate({ id: reward.id, ...values }, {
    ...
    return (
        <div>
            <h2 className="text-2xl font-semibold mb-4">{isEditMode ? 'Edit Reward' : 'Create New Reward'}</h2>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Textarea placeholder="A handful of shiny gold coins." {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                    ...
                    <div className="flex space-x-4">
                        <Button type="submit" disabled={isPending}>{isPending ? 'Saving...' : 'Save Reward'}</Button>
