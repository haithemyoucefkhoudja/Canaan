...
import { useGetAchievements, useDeleteAchievement } from '@/hooks/useAchievements';
...
export default function AchievementClientPage() {
    ...
    if (isFormOpen) {
        return <AchievementForm achievement={selectedAchievement} onFinished={closeForm} />;
    }
                    ...
                    <Card key={achievement.id}>
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div>
                                ...
                                <CardDescription>{achievement.category} - {achievement.xp_bonus} XP</CardDescription>
                            </div>
                            <div className="flex items-center space-x-2">
                                <Button variant="ghost" size="icon" onClick={() => handleEdit(achievement)}>
                                    <Edit className="h-4 w-4" />
                                </Button>
                                <Button variant="ghost" size="icon" onClick={() => handleDelete(achievement.id)} disabled={deleteMutation.isPending}>
