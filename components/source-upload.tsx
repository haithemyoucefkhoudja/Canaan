...
import { Upload, FileText, BookOpen, Globe, FileImage, Calendar, Link, X, Zap } from "lucide-react"
...
export function SourceUpload({ source, onClose }: SourceUploadProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    type: source?.type || "",
    url: source?.url || "",
    publishDate: source?.publishDate ? source.publishDate.toISOString().split("T")[0] : "",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log("Source form submitted:", { formData, linkedEvents, uploadedFile })
    onClose()
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="details" className="space-y-6">
          ...
          <Card className="bg-muted/20">
            <CardHeader>
              <CardTitle className="text-lg">Upload Document (Optional)</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="border-2 border-dashed border-border rounded-lg p-6 text-center">
                  <Upload className="h-8 w-8 mx-auto mb-2 text-muted-foreground" />
                  <p className="text-sm text-muted-foreground mb-2">Upload PDF, Word document, or text file</p>
                  <input
                    type="file"
                ...
                {uploadedFile && (
                  <div className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={extractTextFromFile}>
        ...
        <TabsContent value="content" className="space-y-6">
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Button type="button" variant="outline" size="sm" onClick={generateEmbedding}>
                <Zap className="h-3 w-3 mr-1" />
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Full text enables AI-powered search and analysis. Content will be used to generate embeddings for semantic
              search.
            </p>
          </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">Extract key themes, entities, and concepts</p>
                ...
                <div className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">Generate formatted citations automatically</p>
                  <Button type="button" variant="outline" size="sm">
                    Generate Citation
                  </Button>
                </div>
            ...
            <div className="space-y-4">
              ...
              {linkedEvents.length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-medium">Linked Events ({linkedEvents.length})</h4>
                  <div className="space-y-2">
                    {linkedEvents.map((eventId) => {
                      const event = mockEvents.find((e) => e.id === eventId)
                      if (!event) return null
                      return (
                        <div key={eventId} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                          <span className="font-medium">{event.name}</span>
                          <Button type="button" variant="ghost" size="sm" onClick={() => removeEventLink(eventId)}>
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                      )
          ...
          <Card className="bg-muted/20">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground mb-4">
                Specify how this source relates to the linked events for better organization.
              </p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {["Primary", "Secondary", "Reference", "Context"].map((relevance) => (
                  <Badge
                    key={relevance}
                    variant="outline"
      ...
      <div className="flex justify-end gap-3 pt-4 border-t">
        ...
        <Button type="submit" disabled={!formData.title || !formData.author || !formData.type}>
