...
export function EventsManagement() {
  ...
  const filteredEvents = events
    .filter((event) => {
      ...
      const matchesTag = filterTag === "all" || (event.tags || []).includes(filterTag)
    ...
    return (
      <Card className="bg-card">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <Button onClick={() => queryClient.invalidateQueries({ queryKey: ["events"] })}>Try Again</Button>
        </CardContent>
      </Card>
    )
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage and explore historical events with detailed information and relationships.
          </p>
        </div>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {selectedEvent ? "Update the event information below." : "Add a new historical event to your database."}
              </DialogDescription>
            </DialogHeader>
            <EventForm
              event={selectedEvent}
            ...
            <CardHeader>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <CardTitle className="text-lg mb-2" style={{ fontFamily: "var(--font-heading)" }}>
                    {event.name}
                  </CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <div className="flex items-center gap-1 text-muted-foreground">
                  ...
                  {(event.source_relationships?.length || 0) + (event.target_relationships?.length || 0)} links
                </div>
                <div className="flex items-center gap-1 text-muted-foreground">
              ...
              {event.actorLinks && event.actorLinks.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-sm font-medium text-foreground">Key Actors</h4>
                  <div className="flex flex-wrap gap-1">
                    {event.actorLinks.slice(0, 3).map((link: any) => (
                      <Badge key={link.actor.id} variant="outline" className="text-xs">
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {event.sourceLinks.slice(0, 2).map((link: any) => (
                      <div key={link.source.id} className="text-xs text-muted-foreground truncate">
                        {link.source.title} - {link.source.author}
                      </div>
                    ))}
                    {event.sourceLinks.length > 2 && (
                      <div className="text-xs text-muted-foreground">+{event.sourceLinks.length - 2} more sources</div>
                    )}
                  </div>
                </div>
              )}
